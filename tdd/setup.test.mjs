...
axon@axon   installed, enabled  0.1.7    C:\\Users\\Kevin\\.codex\\plugins\\cache\\axon\\axon\\0.1.7
`;
...
test('parsePluginList reads installed plugin status and version', () => {
  assert.deepEqual(parsePluginList(currentPluginList), {
    ...
    version: '0.1.7',
    path: 'C:\\Users\\Kevin\\.codex\\plugins\\cache\\axon\\axon\\0.1.7',
    installed: true,
  });
});
...
test('setupAxon upgrades stale plugin', () => {
  const calls = [];
  let upgraded = false;
  const stalePluginList = currentPluginList.replaceAll('0.1.7', '0.1.6');
  const result = setupAxon({
    expectedVersion: '0.1.7',
    execCodex: (args) => {
      calls.push(args);
