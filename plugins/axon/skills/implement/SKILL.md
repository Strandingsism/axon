...
Read the plan file from the project-root `.axon/plans/`. Extract every task with its full text. Interface registry and project map are injected by hooks — no need to read them manually.
...
Hooks have reset the project-root `.axon/tasks.json` and injected the task list into context. Create matching tasks in the Codex Task system. After each task completes, update its status in the project-root `.axon/tasks.json` to `"done"`.

Update the project-root `.axon/project-map.md`: mark the `implement` phase as started.
...
After both reviews pass, mark the task complete. Update the project-root `.axon/project-map.md`:
